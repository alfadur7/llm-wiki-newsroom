...
- `dispatch.sh` + `dispatch.py` (the single Write|Edit pre/post + Bash commit-gate dispatcher — parses stdin once, merges simultaneously-firing advisories into a single payload, with a shell fallback when python3 is absent. Consolidated responsibilities:)
  ...
  - advisory: on a `git … commit` in **command position**, names the guideline files that commit carries (same set as the ladder advisory below; a commit pathspec decides alone, else the index, else the paths the command stages) and prints the Guideline Verification Ladder — never a block, and the payload can land after the commit, so it names the not-yet-run branch too
