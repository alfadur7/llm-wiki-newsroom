...
Guards the deterministic parts of corpus ingestion (parse / validate / append)
and clustering (mechanism cluster, recurring-after-fix priority,
addressable=false separation, watermark window).
"""
import json
import sys
...
def test_validate_rejects_bad_kind_and_missing_keys():
    ...
    assert ld.validate({"kind": "defect", "target": "x"})  # mechanism / caught_at missing
    assert ld.validate({"kind": "defect", "target": "x", "mechanism": "m",
                        "caught_at": "lint:source"}) is None


def test_append_fills_date_and_rejects_invalid(tmp_path):
    log = tmp_path / "_defect-log.jsonl"
    n = ld.append_records([{"kind": "defect", "target": "t", "mechanism": "m",
                            "caught_at": "lint:source"}], path=log)
    assert n == 1
...
def _defect(mech, caught_at="desk:밀도", target="t.md", date="2026-06-25", addressable=True):
    return {"kind": "defect", "mechanism": mech, "caught_at": caught_at,
...
def test_recurring_after_fix_ranks_first():
    records = [
        ...
        {"kind": "transition", "cluster": "density-shortfall@desk", "surface": "x",
         "decision": "accept"},
    ]
...
def test_since_window_excludes_old():
    records = [_defect("오래된", date="2026-01-01"), _defect("최근", date="2026-06-25")]
