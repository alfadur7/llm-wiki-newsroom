...
6. In the next cycle, read the new SoT in the GROUND stage and test it. When you have changed guidance that `lint.py` scores (`criteria.json` `judge:A`·`layers/` quantitative rules·`policies/` lint rules·the `lint.py` logic itself), measure both **held-in** (the defect-manifesting target + a fresh sample of an already-treated defect of the same mechanism) and **held-out** (the fixed set in `tools/regression_set.json` + a fresh sample of stable pages of the edit type — both unused as the proposal's motivation, with the sampling rationale audit-logged) before and after the edit, and accept by: acceptance = **≥1 improvement on the motivating held-in** ∧ non-regression across every slice (no PASS→FAIL·over-fire·broad score drop on any side). The fresh samples serve only as non-regression guards; if there are no already-treated defects of that mechanism, run the motivating target alone and downgrade confidence (the sampling rules·audit logging·sparse fallback are in [`../operations/proposal-validation-runbook.md`](../operations/proposal-validation-runbook.md)). Keep the fixed set for longitudinal reproduction but add a fresh sample each cycle to prevent frozen-set overfit (the samples are recorded in the transition record, not grown into the fixed set). lint (deterministic) is measured once, Desk (probabilistic) is aggregated N times — for desk-judged prose guidance (layers craft, etc., not scored by `lint.py`), the blind-rewrite × blind-desk measurement procedure is in [`../operations/proposal-validation-runbook.md`](../operations/proposal-validation-runbook.md). On ADAPT failure, re-update the SoT (3rd FAIL on the same cause → escalate to human)
