...
- 3rd FAIL on the same ADAPT cause · Guideline Verification Ladder not converged after 3 rotations (regardless of cause)
...
The default operating pattern is that the wiki operator does not edit the `.claude/*.md` SoT directly. When the wiki operator delivers feedback·a decision via an utterance within a session, the Editor-in-Chief identifies the exact location in the relevant SoT, updates it, and then tests in the next cycle whether that SoT works as intended. A direct .md-edit path incurs the cost of rediscovering the same feedback in the next session.

Update obligations belong to three responsibilities — **Record · Propose · Gate**.

**An item carrying a timing expression** has its execution time defined by that expression — not by the order of the sub-sections, nor by the order within one.

> A record citing a `step N` item number from an earlier version of this section resolves against the version current at that record's source (by the record's commit or date) — not against HEAD, which misreads text that has changed since.
...
3. **Blind review (mandatory pass)** — hand the diff (diff only, no deliberation context) to a fresh reviewer per the skill's Blind review protocol; the reviewer returns a substantive/invariant classification per hunk + `gdl.*` defects. The reviewer judges only — do not ask it to author the fix (that biases it toward inflating its own findings). Route it to a different model family from the author's ([README § Model routing](README.md#model-routing-frontmatter-model)) — this reviewer is spawned ad hoc, so nothing but the invocation sets its family
   - **ADAPT** — fix critical·high only. Before fixing, re-read each touched file whole; then search those files for the pattern the defect defines and dispose of the residual instances at your discretion — closing only the cited instance leaves its twins for the next rotation, which a linear re-read does not catch
   - **medium·low carry over to the corpus** instead of another edit round (batch-loaded via `log_defect` at cycle close) — churn control
   - **Re-pass** — after a fix, re-run the pass on the cumulative diff, handing the reviewer the previous round's findings verbatim (blindness withholds the author's intent, not the prior reviewer's output). Immediate-fix scope narrows to ① the original finding still open and ② **critical·high** sitting in text this round's fix wrote; critical is fixed wherever it sits, everything else carries over at its severity
   - **Rotation cap 3, counting the first pass** — the pass ends when that immediate-fix set is empty; if it has not by the 3rd, stop fixing, hold the commit, and escalate to the wiki operator — **the operator decides what to revert and whether to extend the rotations.** Fix byproducts surface under a different cause each round, slipping past the same-cause counter in [README § ADAPT Escalation](README.md#adapt-escalation), and an author reverting the whole batch alone discards the parts already verified
4. **Effect-measurement gate** — when rung 3 classified a hunk substantive AND the change is of a measurement-obligated type ([runbook § When to Run](../operations/proposal-validation-runbook.md)), an accept transition must exist in the corpus before commit; otherwise hold the commit, revert (or isolate) the edit, and then run the measurement — that revert is a temporary isolation for the measurement and is re-applied on acceptance, since staged guideline text contaminates its own measurement (acceptance rule: § Gate above; per-type procedure: the runbook's three variants)
