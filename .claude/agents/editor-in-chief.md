...
3. **Blind review (mandatory pass)** — hand the diff (diff only, no deliberation context) to a fresh reviewer per the skill's Blind review protocol; the reviewer returns a substantive/invariant classification per hunk + `gdl.*` defects. The reviewer judges only — do not ask it to author the fix (that biases it toward inflating its own findings)
   ...
   - **Rotation cap 3, counting the first pass** — the pass ends when that immediate-fix set is empty; if it has not by the 3rd, revert the edit and escalate to the wiki operator. Fix byproducts surface under a different cause each round, slipping past the same-cause counter in [README § ADAPT Escalation](README.md#adapt-escalation)
