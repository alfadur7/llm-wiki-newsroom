...
- **Model routing**: these agents are spawned by the script, so the frontmatter pins in [`agents/README.md` § Model routing](../agents/README.md#model-routing-frontmatter-model) never reach them — pass `model` on every `agent()` call or each stage silently takes the session default. That section's family split is not carried into the review and verify stages as a rule; default to the skeleton's constants and deviate on capability or cost with the reason recorded in the run.
