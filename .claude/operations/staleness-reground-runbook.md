...
- **timeline · trail** (low-risk, structural): a single columnist re-ground pass (new events · hops · refreshed commentary) plus a deterministic and spot review in the main session. Skip the full desk gate to save cost, but cross-check each new timeline item's date and source index against the original one by one.
