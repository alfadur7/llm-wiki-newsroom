...
The `## Human Reviewer Gate` section of each command file is interpreted as the **union of the CLAUDE.md global gates and this command's specific gates**. The global gates have their single SoT in [`CLAUDE.md` "Human Reviewer Gate"](../../CLAUDE.md#human-reviewer-gate) — each command file does not re-list these gates and specifies **only the specific gates** that arise within its own trigger area.
