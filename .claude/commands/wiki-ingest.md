...
3. **Proceed in folder mode** — the 13-step procedure on the fetch outputs (`raw/NewsScrap`·`raw/PDF`). **Create source pages only after the raw has landed on disk** — because the raw tree is the dedup ground truth (`by_path`), an unfetched source bypasses duplicate detection.
...
5. **Update `wiki/overview.md`** — revise if the synthesis change is worth it
6. **Entity/concept pages (upsert after duplicate check)**:
...
7. **Cascade update** — add sources + strengthen facts + update last_updated on existing hubs linked in the source's `## Connections`. On bulk ingest, this may be skipped and post-processed by lint.
8. **Flag contradictions** — identify contradictions with existing wiki content
9. **`log.md` append**: `## [today's date] ingest | <Title>`
10. **Automatic verification** (required):
...
11. **Full rebuild + cluster diagnosis**:
...
12. **L2-1 source Desk VERIFY₂ (sub-trigger)** — extract the source list meeting `[fact]≥7 AND citations≥3` from the `python tools/lint.py source` Advisory output, then the Editor-in-Chief invokes the Desk in batch. The Desk returns a defect list → Reporter ADAPT₂ → re-lint. SoT: [`agents/desk.md`](../agents/desk.md) L2-1 source VERIFY₂.
13. **Cycle defect batch-logging** — batch-log this cycle's escaped lint FAILs and Desk-actionable defects once into the `tools/log_defect.py` corpus (`tools/_defect-log.jsonl`) (`caught_at`=`<stage>:<detail>`·`mechanism`·`severity`·`addressable`). `mine_failures` bundles end-to-end recurrence rates and feeds them into the SoT self-evolution automatic channel. SoT: [`agents/editor-in-chief.md`](../agents/editor-in-chief.md).
