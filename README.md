...
- **The recorded rung** — every hand-off notes where the writer stopped (`grounded_at`), and the defect corpus stores it alongside *which review surface caught the defect*. That second coordinate is the point: without it, a defect born from under-reading is indistinguishable from bad writing, so the meta loop's only possible prescription was "fix the writing rules." Recording the pair is what makes "widen the reading earlier" available as a diagnosis at all — drawing it is a separate step, and not built.

The design follows the ablation result of an independently built system of the same shape (see [How it differs from RAG](#how-it-differs-from-rag) below). *(New and unproven: the stopping rules are provisional placeholders, nothing consumes the recorded rungs yet, and no effect has been measured. What has arrived is the record — **343 hand-offs across 21 cycles on the same private instance, 87% of which never left R0, with the top rung never once fired.** A lopsided distribution is not yet a verdict. It could mean the ladder is correctly cheap, that the climb rarely triggers, or that the signals meant to trigger it go unchecked. Whether that calibrates the upper rungs or kills them is the open question.)*
...
- **Two isomorphic verification ladders** — wiki content climbs a Content Verification Ladder (post-edit hook → self-lint → batch lint → qualitative desk review → publish gate), and changes to the writing rules themselves climb a matching Guideline Verification Ladder (deterministic lint → minimal-edit check → blind review → an effect-measurement gate, where the change is of a kind that gate covers). The same "cheapest deterministic check first" principle governs both layers.
...
- Proposals aren't applied immediately. Where a change touches the prose an LLM judge scores, it must show a **measured effect**: the same task is rewritten side-by-side with and without the change and scored blind, and a held-out regression set confirms nothing else got worse. That covers a minority of changes — the rest are verified deterministically, or adopted on an operator directive that waives the measurement and records why.
