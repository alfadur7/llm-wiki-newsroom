...
[카파시의 LLM Wiki](https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f)가 제시한 3계층 골격을 그대로 씁니다. 원본을 손대지 않고 두는 `raw/`, 에이전트가 관리하는 `wiki/`, 운영 규칙을 담은 스키마 계층입니다. 문서를 넣고(ingest), 물어보고(query), 점검하는(lint) 세 가지 작업도 그대로입니다. 이 아이디어를 구현한 사례는 이제 많은데, 그중 인기 있는 것들을 살펴보니 본 프로젝트의 차별점은 세 가지입니다.
