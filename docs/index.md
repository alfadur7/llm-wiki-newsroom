...
- **Authoring guidelines that evolve themselves.** When the same review failure keeps recurring, the system drafts a fix to its _own_ writing rules. Where the fix touches prose an LLM judge scores, it has to clear a blind A/B against a rotating regression set first; the rest is verified deterministically, and the operator can waive the A/B by directing a change, with the reason recorded. Either way the operator signs off before anything is adopted; the loop never adopts on its own. So it isn't only the wiki that improves over time, but the rules that build it. *(Thirteen weeks of that ledger on a private instance: 107 adopted, 72 rejected, 18 deferred — the A/B is cited in 16 of the 107, 40 were adopted on a recorded waiver, and the rest are changes it doesn't cover.)*
...
- **The differentiators are hypotheses.** Writer–reviewer separation and the self-evolving loop are argued from design, not from published A/B numbers. The adoption and rejection counts above are a record of the loop running, not evidence that it works. On the instance behind those figures, 44 of the 70 defect classes that ever got a fix came back afterwards, and five of them produced 451 of the 823 recurrences (55%). The newest mechanism, a ladder governing how much the writer reads before drafting, ships with provisional stopping rules and no effect measured at all.
